...
Existing export files are preserved unless --force is explicitly supplied.`);
  ...
  const machine = MachineSchema.safeParse({
    id: values.machine,
    member: values.member,
    label: values.label ?? values.machine,
  });
  if (!machine.success)
    throw new Error(
      "Provide --machine (stable identifier) and --member. Run --help for usage.",
    );
  const roots: { provider: Provider; path: string }[] = [
    ...(values["claude-root"] ?? []).map((path) => ({
      provider: "claude-code" as const,
      path,
    })),
    ...(values["codex-root"] ?? []).map((path) => ({
  ...
  if (!roots.length)
    throw new Error(
      "Choose --claude-root or --codex-root explicitly. No directories were scanned.",
    );
  const result = await collect({
    roots,
    machine: machine.data,
    includePrompts: values["include-prompts"],
  });
  const output =
    values.out ?? join("exports", `${machine.data.id}-${Date.now()}.json`);
  await writeBundle(output, result.bundle, values.force);
  console.log(
    `Saved ${result.bundle.usage.length} usage records and ${result.bundle.prompts.length} prompts from ${result.filesRead} files to ${output}.`,
  );
  console.log(
    `${result.duplicates} duplicates skipped. ${result.diagnostics.length} diagnostics (up to 200 shown).`,
  );
...
main().catch((error: unknown) => {
  if (error instanceof Error && "code" in error && error.code === "EEXIST") {
    console.error(
      "Export already exists. Choose a different --out path or explicitly pass --force.",
    );
  } else
    console.error(
